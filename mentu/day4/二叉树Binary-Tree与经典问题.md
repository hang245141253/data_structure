...
其实不是二叉树的题，而是一个dp题。
    ...
    则：
    dp[0][0]代表父节点不放置、本节点也不放置的这种情况下，想覆盖本节点所在的子树中所有节点的最少摄像头放置的数量。 （摄像头红圈的范围最少的数量）
    以此类推

![alt text](image-30.png)


强调dp数组的定义，也是在强调递归函数的意义。每一个节点都有属于他自己的dp数组，


    下图表示，对于某个节点，当改节点不放摄像头，父节点也不放摄像头，对于这种情况，dp[0][0]的最小值应该是由这三种情况决定：
        l[0][1]+r[0][0]
        l[0][0]+r[0][1]
        l[0][1]+r[0][1]
    取三种情况最小值，只有这三种放置方案
...
public:
    ...
    int minCameraCover(TreeNode* root) {
        ...
        return min(dp[0][0], dp[0][1]); // 从该节点放与不放的方案中选取最小的方案
        // 这里是站在根节点的角度，肯定没有父节点
    }
};
```
